...
import { searchEmailForgetPassword } from "../Services/authService";
...
const SearchEmail = () => {
  ...
  const onSubmit = async (data) => {
    try {
      setLoading(true);
      const res = await searchEmailForgetPassword(data);
      if (res?.status === "success") {
        toast.success(
          t("OTP has been sent to your email. Use it to Forget password.")
        );
        localStorage.setItem("forget", JSON?.stringify(res?.email));
  ...
  useEffect(() => {
    ...
    try {
      const decryptedData = atob(encryptedToken).replace("123abc", "");
      const data = JSON.parse(atob(decryptedData));
      setChangePassword(data?.data?.isChangePassword);
    } catch (error) {
