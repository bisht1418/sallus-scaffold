...
import { dummyUsers } from "../DummyData/DashboardCardInfoDummyData";
...
const Users = () => {
  return (
    <div>
      <AdminDashboard data={AllUsersProps} />
    </div>
  );
};
...
const AllUsersProps = () => {
  ...
  const getAllUsers = async () => {
    try {
      setIsLoading(true);
      if (isAdminId) {
        const getAllUsersResponse = await getUserById(isAdminId);
        console.log(getAllUsersResponse, "getAllUsersResponse");
        setUsers(getAllUsersResponse.data);
      } else {
        const getAllUsersResponse = await getAllUsersService();
        const allUsers = getAllUsersResponse.data || [];
        const filterAdminData = allUsers?.filter((ele) => ele.type === 1);
        setUsers(filterAdminData);
      }
    } catch (error) {
      setIsLoading(false);
    } finally {
  ...
  return (
    <div className="p-4 mt-5 mb-10">
      <p className="text-xl font-bold text-gray-600 uppercase">Users</p>
