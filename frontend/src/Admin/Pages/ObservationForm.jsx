...
const ObservationForms = () => {
  return (
    <div>
      <AdminDashboard data={ObservationFormProps} />
    </div>
  );
};

const ObservationFormProps = () => {
  ...
  const getAllForms = async () => {
    try {
      ...
      if (isAdminId) {
        const responseData = await getObservationByUserId(isAdminId);
        const getAllUsersResponse = responseData?.data;
        const observations = getAllUsersResponse?.filter(
          (ele) => !ele?.isDeleted
        );
        const allForms = [...observations] || [];
        setForms(allForms);
      } else {
  ...
  const handleConfirmationOperation = async () => {
    ...
    try {
      if (operationType === "delete" && currentItem) {
        ...
        const deleteResponse = await deleteObservation(currentItem?._id);
        ...
        let responseSearch = forms.filter((form) => {
          ...
          if (form.isObservation) {
            ...
            projectNumber = form?.projectId?.projectNumber
              .toLowerCase()
              .includes(searchTerm);
            projectName = form?.projectId?.projectName
              .toLowerCase()
              .includes(searchTerm);
          }
