...
import { deleteSafeJobAnalysis } from "../../Services/safeJobAnalysisService";
...
const SjaForms = () => {
  return (
    <div>
      <AdminDashboard data={SjaFormProps} />
    </div>
  );
};
...
const SjaFormProps = () => {
  ...
  const getAllForms = async () => {
    try {
      ...
      if (isAdminId) {
        const responseData = await getSJAByUserId(isAdminId);
        const getAllUsersResponse = responseData?.data;
        const safeJobAnalyses = getAllUsersResponse?.filter(
          (ele) => !ele?.isDeleted
        );
        const allForms = [...safeJobAnalyses] || [];
        setForms(allForms);
      } else {
  ...
  const handleConfirmationOperation = async () => {
    ...
    try {
      if (operationType === "delete" && currentItem) {
        const deleteResponse = await deleteSafeJobAnalysis(currentItem?._id);
