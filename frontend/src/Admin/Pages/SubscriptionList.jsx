...
const SubscriptionList = () => {
  return (
    <div>
      <AdminDashboard data={SubscriptionProps} />
    </div>
  );
};
...
const SubscriptionProps = () => {
  ...
  const getAllUsers = async () => {
    try {
      ...
      if (isAdminId) {
        getAllUsersResponse = await getAllUsersService();
        const allUsers = getAllUsersResponse?.data;
        const filterAllUserData = allUsers?.filter(
          (ele) => ele?.createdBy === isAdminId
        );
        setUsers(filterAllUserData);
      } else {
        getAllUsersResponse = await getAllUsersService();
        const allUsers = getAllUsersResponse?.data;
        const filterAllUserData = allUsers?.filter((ele) => ele.isSubscription);
        setUsers(filterAllUserData);
      }
    } catch (error) {
  ...
  return (
    <div className="p-4 mt-5 mb-10">
      <p className="text-xl font-bold text-gray-600 uppercase">Subscription</p>
