...
import { getAllFormService } from "../../Services/AdminService/allFormsService";

const AllForms = () => {
  return (
    <div>
      <AdminDashboard data={AllFormProps} />
    </div>
  );
};
...
const AllFormProps = () => {
  ...
  const getAllForms = async () => {
    try {
      ...
      const approvalForms = getAllUsersResponse?.approvalForms;
      const observations = getAllUsersResponse?.observations;
      const materialListWithProjects =
        getAllUsersResponse?.materialListWithProjects;
      const afterControlForms = getAllUsersResponse?.afterControlForms;
      const safeJobAnalyses = getAllUsersResponse?.safeJobAnalyses;
    ...
    debounceTimer = setTimeout(async () => {
      try {
        let responseSearch = forms.filter((form) => {
          ...
          if (form.isApprovalForm) {
            nameMatch = form?.scaffoldName[0].value
              .toLowerCase()
              .includes(searchTerm);
            projectNumber = form.userId.phoneNumber
              .toLowerCase()
              .includes(searchTerm);
          } else if (form.isObservation) {
            nameMatch = form?.observerDetails.observerDetail
              .toLowerCase()
              .includes(searchTerm);
            projectNumber = form.projectNumber
              .toLowerCase()
              .includes(searchTerm);
          } else if (form.isMaterialListForm) {
            projectNumber = form.projectNumber
              .toLowerCase()
              .includes(searchTerm);
          } else if (form.isAfterControlForm) {
            projectNumber = form.projectNumber
              .toLowerCase()
              .includes(searchTerm);
          } else if (form.isSafeJobAnalyses) {
            projectNumber = form.projectNumber
              .toLowerCase()
              .includes(searchTerm);
          }
  ...
  return (
    <div className="p-4 mt-5 mb-10">
      <p className="text-xl font-bold text-gray-600 uppercase">All Forms</p>
