...
import { deleteApprovalFormService } from "../../Services/approvalFormService";
...
const ApprovalForms = () => {
  return (
    <div>
      <AdminDashboard data={ApprovalFormProps} />
    </div>
  );
};

const ApprovalFormProps = () => {
  ...
  const getAllForms = async () => {
    try {
      ...
      let responseData;
      if (isAdminId) {
        responseData = await getApprovalFormByUserId(isAdminId);
        const getAllUsersResponse = responseData?.data;
        const approvalForms = getAllUsersResponse.filter(
          (ele) => !ele.isDeleted
        );
        const allForms = [...approvalForms] || [];
        setForms(allForms);
      } else {
  ...
  const handleConfirmationOperation = async () => {
    ...
    try {
      if (operationType === "delete" && currentItem) {
        const deleteResponse = await deleteApprovalFormService(
          currentItem?._id
        );
      ...
      try {
        let responseSearch = forms.filter((form) => {
          ...
          if (form.isApprovalForm) {
            nameMatch =
              form?.scaffoldName?.[0]?.value
                ?.toLowerCase()
                .includes(searchTerm) ?? false;
            projectNumberMatch =
              form?.projectId?.projectNumber
                ?.toLowerCase()
                .includes(searchTerm) ?? false;
            projectNameMatch =
              form?.projectId?.projectName
                ...
                .includes(searchTerm) ?? false;
            emailMatch =
              form?.userId?.email?.toLowerCase().includes(searchTerm) ?? false;
          }
