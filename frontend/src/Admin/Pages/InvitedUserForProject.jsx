...
import {
  getAllInviteForm,
  getInviteFormByUserId,
} from "../../Services/AdminService/inviteUserProject";
import { useNavigate } from "react-router-dom";

const noDataImage = require("../../Assets/no_data_found.webp");

const InvitedUserForProject = () => {
  return (
    <div>
...
const InviteUserForProjectProps = () => {
  ...
  const getAllProject = async () => {
    try {
      ...
      if (isAdminId) {
        getAllProjectResponse = await getInviteFormByUserId(isAdminId);
        const allProject = getAllProjectResponse.data?.filter(
          (ele) => !ele.isDeleted
        );
        setProjects(allProject);
      } else {
        getAllProjectResponse = await getAllInviteForm();
        const allInviteResponse = getAllProjectResponse?.Invite;
        const allProject = allInviteResponse?.filter((ele) => !ele.isDeleted);
        setProjects(allProject);
      }
    } catch (error) {
  ...
  const handleConfirmationOperation = async () => {
    ...
    try {
      if (operationType === "delete" && currentItem) {
        // const deleteResponse = await projectDeleteService(currentItem?._id);
  ...
  const delayedAPICall = (term) => {
    ...
    debounceTimer = setTimeout(async () => {
      try {
        let responseSearch = projects.filter((project) => {
          const searchTerm = term.toLowerCase();

          const nameMatch = project.projectName
            .toLowerCase()
            .includes(searchTerm);
          const emailMatch = project.projectDetails.project_email
            .toLowerCase()
            .includes(searchTerm);
          const phoneMatch = project.projectDetails.project_phone_number
            .toLowerCase()
            .includes(searchTerm);
          ...
          return nameMatch || emailMatch || phoneMatch || projectNumber;
        });
  ...
  return (
    <div>
      <div className="border p-4 m-4 rounded-xl bg-white overflow-auto h-[88.5vh]">
        <p className="text-xl font-bold uppercase ">Invited Users List</p>
        <div className="p-4">
                      ...
                      <tr key={index} className="bg-white border-b">
                        ...
                        <th className="px-6 py-4 font-semibold ">
                          <div className=" flex  flex-col">
                            <p>{item?.userId?.name || "No Data Available"}</p>
                            <p>{item?.userId?.email || "No Data Available"}</p>
                          </div>
                        </th>
...
export default InvitedUserForProject;
