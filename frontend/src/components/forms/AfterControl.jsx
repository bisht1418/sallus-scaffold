...
const AfterControl = ({ approvalForm }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getAfterControlFormService(projectId);
      const responseData = res?.data;
      const filteredData = responseData?.filter(
        (ele) => !ele?.isDeleted && ele?.status === "active"
      );

      console.log("filteredData", filteredData);

      const updatedFilteredData = filteredData.map((item) => {
        const updatedVisual = item.visual
          .filter((subItem) => subItem.inspection !== "na") // Filter out 'na' items
          .map((subItem) => ({
            ...subItem,
            inspection: null,
            documentFile: null,
      ...
      setAfterControl(updatedFilteredData);
    } catch (error) {
  ...
  useEffect(() => {
    fetchData();
    const controlNameAssign = formatDateString();
    setControlName(controlNameAssign);
  }, []);
  ...
  const projectName = afterControl
    ? afterControl?.map((projectName) => projectName?.projectId?.projectName)
    : null;

  const projectDate = afterControl
    ? afterControl?.map((projectName) => projectName?.date)
    : null;
  ...
  const sendDataToBackend = async () => {
    ...
    if (signature?.approver?.signature) {
      const response = await createAfterControlFormService(payload);
  ...
  return (
    <>
      ...
      <div className="custom-container flex  flex-col mb-4">
        ...
        <input
          className="!pl-2 !w-1/2 border !border-black !outline-none font-bold text-md"
          type="text"
          value={controlName}
          readOnly
        />
      ...
      <div className="custom-container">
        <table className="border-collapse w-full cursor-pointer">
          <thead className="border-b-2 border-b-[#CCCCCC]">
            <tr>
              <th className="sm:p-4 py-2 px-3 uppercase">{projectName[0]}</th>
              <th className="sm:p-4 py-2 px-3 uppercase">
                {t("Activescaffolds")}
              </th>
            </tr>
          </thead>
