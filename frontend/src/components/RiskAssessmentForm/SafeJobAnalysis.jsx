...
import { createSafeJobAnalysis } from "../../Services/safeJobAnalysisService";
import { useParams } from "react-router-dom";
import { getProjectByIdService } from "../../Services/projectService";
import { toast } from "react-toastify";
import axios from "axios";
import { useNavigate } from "react-router-dom";
import SignaturePersonModal from "../SignaturePersonModal";
import { useSelector } from "react-redux";
import { t } from "../../utils/translate";
import CloseIcon from "../../Assets/iconclose.svg";
import SafetyChecklist from "../SafetyChecklist";
import RequiredPermitChecklist from "../RequiredPermitChecklist";
import { AiOutlineSafety } from "react-icons/ai";
import DesignatedWorkChecklist from "../DesignatedWorkChecklist";
...
const SafeJobAnalysis = () => {
  ...
  const defaultItems = [
    { checkListName: "sja_hard_hat", yes: false, na: false, no: false },
    {
      checkListName: "sja_safety_glassess",
      yes: false,
      na: false,
      no: false,
    },
    { checkListName: "sja_gloves", yes: false, na: false, no: false },
    { checkListName: "sja_safety_boots", yes: false, na: false, no: false },
    {
      checkListName: "sja_high_visibility_cloths",
      yes: false,
      na: false,
      no: false,
    },
    {
      checkListName: "sja_hearing_protection",
      yes: false,
      na: false,
      no: false,
    },
    { checkListName: "sja_dust_mask", yes: false, na: false, no: false },
    {
      checkListName: "sja_protection_harness",
      yes: false,
      na: false,
      no: false,
    },
  ...
  const defaultRequirePermit = [
    { checkListName: "sja_work_height", yes: false, no: false },
    { checkListName: "sja_confined_space", yes: false, no: false },
    { checkListName: "sja_hot_work", yes: false, no: false },
  ];

  const defaultDesignatedWorkArea = [
    { checkListName: "sja_cordon", yes: false, no: false },
    { checkListName: "sja_question_01", yes: false, no: false },
    {
      checkListName: "sja_question_02",
      yes: false,
      no: false,
    },
    {
      checkListName: "sja_question_03",
      yes: false,
      no: false,
    },
    {
      checkListName: "sja_question_04",
      yes: false,
      no: false,
    },
  ];
  ...
  const [designatedWorkArea, setDesignatedWorkArea] = useState(
    defaultDesignatedWorkArea
  );
  const [extendedList, setExtendedList] = useState(true);

  const [showConfirmationModal, setShowConfirmationModal] = useState(false);
  const [currentEditedId, setCurrentEditedId] = useState("");
  const [editedData, setEditedData] = useState("");
  const [editCreateTask, setEditCreateTask] = useState({
    risk: "",
    measure: "",
    riskLevel: "",
    measureLevel: "",
  ...
  const handleDeleteSignature = (name, index) => {
    const updatedPersonSignatures = [...personSignatures];
    if (index >= 0 && index < updatedPersonSignatures?.length) {
      updatedPersonSignatures.splice(index, 1);
      setPersonSignatures(updatedPersonSignatures);
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (personSignatures?.length > 0) {
        signature = {
          ...signature,
          person: personSignatures,
        };
      } else {
      ...
      const payload = {
        ...
        protectiveEquipment: items,
        typeOfWork: requiredPermit,
        designatedWorkArea: designatedWorkArea,
        emergencyProcedure: data?.emergencyProcedure,
      };

      const response = await createSafeJobAnalysis(payload);
  ...
  const handleUpload = async (base64Image) => {
    ...
    try {
      ...
      const publicId = `users/${projectDetail?.userId}/${folderName}/${
        addData.name || `${projectDetail?.userId}_signature`
      }`;
  ...
  const handleMeasure = (index, event, isMeasureLevel) => {
    const newData = [...data];
    if (isMeasureLevel) {
      newData[index].measureLevel = event.target.value;
      setData(newData);
    } else {
  ...
  const handleAddDiv = () => {
    ...
    setData([
      ...data,
      { risk: "", measure: "", riskLevel: "", measureLevel: "" },
    ]);
  };
  ...
  const handleEdit = (id) => {
    const updatedFormData = [...scaffoldData];
    const filteredData = updatedFormData?.filter((ele, ind) => ind === id);
    setEditedData([...filteredData[0]?.subtaskData] || []);
  ...
  const handleEditTaskDelete = (id) => {
    const deleteData = [...editedData]?.filter((ele, ind) => ind !== id);
    setEditedData(deleteData);
  };

  const handleEditTask = () => {
    const addTaskToMainList = [...editedData, editCreateTask];
    setEditedData(addTaskToMainList);
    setEditCreateTask({
      risk: "",
      measure: "",
      riskLevel: "",
  ...
  const handleEditMainTask = () => {
    ...
    setScaffoldData((prev) =>
      prev.map((ele, id) => {
        if (id === currentEditedId) {
          return { ...ele, subtaskData: newEditedData };
        } else {
          return ele;
        }
      })
  ...
  return (
    <div>
      <div className="md:pb-[50px] pb-6 border-b-[#cccccc] border-b">
        <div className="custom-container flex flex-col md:flex-row gap-[20px] justify-between ">
          <p className="title-text ">{t("safeJobAnalysis")}</p>
          <div className="flex gap-4">
            <div className="flex justify-center items-center gap-[1rem]">
              <div className="flex justify-between rounded-[5px] items-center gap-[30px] border border-[rgb(204, 204, 204)] px-[10px] py-[11px] bg-[white]">
                <p className="project-number leading-0 uppercase">
                  {t("projectName")}
                </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="custom-container">
          <div className="flex flex-col gap-[30px]">
            <div className="flex justify-between items-center md:py-[50px] sm:py-8 py-4">
              <p className="medium-title text-center underline w-full">
                {t("safeJobAnalysis")}
              </p>
            </div>
          </div>
              ...
              <div className="">
                <input
                  ...
                  onChange={(e) =>
                    setRiskAssessment({
                      ...riskAssessment,
                      safeJobAnalysisName: e.target.value,
                    })
                  }
                ...
                {errors?.projectDetail?.safeJobAnalysisName && (
                  <span style={styles} className="mt-2 block">
                    {errors?.projectDetail?.safeJobAnalysisName?.message}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="my-8">
                    ...
                    {scaffoldData?.map((element, index) => (
                      <React.Fragment key={index}>
                        ...
                        >
                              ...
                              element.subtaskData.map((data, dataIndex) => (
                                ...
                                >
                                  <li className="sm:text-base text-sm font-medium">
                                    {data.measure}{" "}
                                    <span>
                                      {data?.measureLevel === "low"
                                        ? "🟢"
                                        : data?.measureLevel === "medium"
                                        ? "🟡"
                                        : data?.riskLevel === "high"
                                        ? "🔴"
                                        : ""}
                          ...
                          <td className="sm:p-4 py-2 px-3 text-center flex gap-6">
                            <button
                              onClick={(e) => {
                                e.preventDefault();
                                handleEdit(index);
                            ...
                            >
                              <FaPencilAlt color="green" size={20} />
                            </button>
                            <button
                              onClick={(e) => {
                                e.preventDefault();
            ...
            <lable className="project-number w-full">{t("addSubtask")}</lable>
            <div className="flex gap-3 mt-2">
              <input
                type="text"
                className="w-32 p-2 border  input-without-icon"
              ...
              <button
                className={`button-text ${
                  selectedSubTask?.length <= 0 ? `bg-[gray]` : `bg-[#0072BB]`
                }
               ${
                 selectedSubTask?.length <= 0
                   ? `hover:bg-[gray]`
                   : `hover:bg-[#0073bbde]`
               } transition-all delay-100 text-[white] px-[20px] py-[10px] rounded-[5px] text-nowrap`}
                onClick={(e) => {
                  e.preventDefault();
                  handelAdd();
                }}
            ...
            {addSubtak && (
              <div>
                {[...Array(divCount)].map((_, index) => (
                  <div
                    className="flex sm:flex-row flex-col justify-between items-center w-full  gap-5 mt-5"
                    key={index}
                  >
                    <div className="flex flex-col justify-start items-start  gap-3  w-full">
                      <label className="project-number">{t("Risk")}</label>
                        ...
                        <div className="relative inline-block w-64">
                          ...
                          <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                            <svg
                              ...
                              viewBox="0 0 20 20"
                            >
                              <path d="M14.348 7.852a.5.5 0 0 0-.697-.716L10 10.788 6.348 7.136a.5.5 0 1 0-.696.717l4 4a.5.5 0 0 0 .696 0l4-4z" />
                            </svg>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col justify-start items-start  gap-3  w-full">
                      <label className="project-number">Measures</label>
                      <div className="flex justify-center items-center gap-5">
                        <input
                          type="text"
                          className="w-32 p-2 border rounded-md shadow-md input-without-icon"
                        ...
                        <div className="relative inline-block w-64">
                          <select
                            onChange={(event) =>
                              handleMeasure(index, event, true)
                            }
                            className="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline"
                          >
                            <option value="">🌟 {t("riskLevel")}</option>
          ...
          <div className="w-full text-center">
            <button
              ...
              className="font-bold text-sm px-4 py-2 rounded-lg bg-[#0081c8] text-white mb-5 "
            >
              {t("extendedList")}
            </button>
          </div>
          ...
          <div
            className={`transition-all duration-500 ease-in-out overflow-hidden ${
              extendedList ? "max-h-0 opacity-0" : "opacity-100"
            }`}
          >
            <div className="sm:p-10 mt-10 border-t-2">
              <SafetyChecklist setItems={setItems} items={items} />
            </div>

            <div className="sm:p-10 mt-10 border-t-2">
              <RequiredPermitChecklist
                setRequiredPermit={setRequiredPermit}
                requiredPermit={requiredPermit}
              />
            </div>

            <div className="sm:p-10 mt-10 border-t-2 border-b-2">
              <DesignatedWorkChecklist
                setDesignatedWorkArea={setDesignatedWorkArea}
                designatedWorkArea={designatedWorkArea}
              />
            </div>
      ...
      {showConfirmationModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-80  z-40 ">
          <div className="relative sm:w-[600px] w-auto  min-h-[600px] h-auto bg-slate-100 rounded-md mx-2 ">
            <IoClose
              onClick={() => setShowConfirmationModal(false)}
              className="absolute top-[3%] right-[2%] text-[26px] cursor-pointer rounded-full border-2 font-bold"
            />
            <button
              onClick={() => setShowConfirmationModal(false)}
            ...
            <div class="relative overflow-x-auto sm:rounded-lg mt-20 px-2">
                  ...
                  {editedData.map((ele, id) => (
                    <tr class="bg-white border-b   ">
                      ...
                      <td class="px-6 py-4">
                        {ele.measure}{" "}
                        <span>
                          {ele?.measureLevel === "low"
                            ? "🟢"
                            : ele?.measureLevel === "medium"
                            ? "🟡"
                            : ele?.measureLevel === "high"
                            ? "🔴"
                            : ""}
                        </span>
                      </td>
              ...
              <div className=" mt-10">
                <div className=" ">
                  <div className="flex items-center justify-between gap-5">
                    <div>
                      <label className="font-bold text-sm">{t("Risk")}</label>
                      <input
                        type="text"
                        className=" !h-10 p-2  rounded-md shadow-md input-without-icon"
                        value={editCreateTask.risk}
                        onChange={(e) =>
                          setEditCreateTask({
                            ...editCreateTask,
                            risk: e.target.value,
                    ...
                    <div className=" ">
                      <div className="font-bold text-sm">{t("addSubtask")}</div>
                      <select
                        value={editCreateTask.riskLevel}
                        onChange={(e) =>
                          setEditCreateTask({
                            ...editCreateTask,
                        ...
                        className="block appearance-none w-full bg-white  border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline"
                      >
                        <option value="">🌟 {t("riskLevel")}</option>
                        <option value="low">🟢 Low</option>
                        <option value="medium">🟡 Medium</option>
                ...
                <div className="flex items-center justify-between gap-5">
                  <div>
                    <label className="font-bold text-sm">Measure</label>
                    <input
                      type="text"
                      className=" !h-10 p-2  rounded-md shadow-md input-without-icon"
                      value={editCreateTask.measure}
                      onChange={(e) =>
                        setEditCreateTask({
                          ...editCreateTask,
                  ...
                  <div className="">
                    <div className="font-bold text-sm">Measure Level</div>
                    <select
                      value={editCreateTask.measureLevel}
                      onChange={(e) =>
                        setEditCreateTask({
                          ...editCreateTask,
                          measureLevel: e.target.value,
                        })
                      }
                      className="block appearance-none w-full bg-white  border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline"
                    >
                ...
                <div className="flex justify-between">
                  <button
                    ...
                    className="mt-2 font-bold text-white bg-[#0081c8] rounded-lg py-2 px-4"
                  >
                    {t("add")}
                  </button>
                  <button
